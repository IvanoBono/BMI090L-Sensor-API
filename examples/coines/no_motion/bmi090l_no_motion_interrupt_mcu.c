...
volatile bool no_motion_int_status = 0;
...
/*!
 * @brief    BMI090L no-motion interrupt callback
 */
void no_motion_int_callback(void);
...
static void configure_bmi090l_no_motion_interrupt(void)
{
    ...
    no_motion_cfg.threshold = 0xAA; /* 83mg */
    no_motion_cfg.duration = 5;
...
int main(int argc, char *argv[])
{
    ...
    configure_bmi090l_no_motion_interrupt();

    coines_attach_interrupt(COINES_SHUTTLE_PIN_21, no_motion_int_callback, COINES_PIN_INTERRUPT_FALLING_EDGE);

    while (1)
    {
        if (no_motion_int_status == true)
        {
            no_motion_int_status = false;
            printf("No motion detected %lu \n", interrupt_count);
        }
    }
